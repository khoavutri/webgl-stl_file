...
var loadVolume = function (selection, onload) {
  ...
  req.onprogress = function (evt) {
    ...
    loadingProgressBar.setAttribute(
      "style",
...
var renderLoop = function () {
  ...
  if (currentIsovalue != isovalue.value || newVolumeUpload) {
    ...
    if (useWebASM.checked) {
      ...
      triangles = marchingCubes.marching_cubes(
        parseFloat(currentIsovalue / 255.0)
      );
    ...
    isosurfaceInfo.innerHTML =
      `Isosurface at ${currentIsovalue} contains ${isosurfaceNumVerts / 3} ` +
...
const sixTool = document.getElementById("vipvipvivpvzczxc");

function exportSTLFromVerticesAndFaces(vertices, faces) {
  // Tạo một geometry mới
  const geometry = new THREE.BufferGeometry();

  // Chuyển đổi các đỉnh thành Float32Array
  ...
  // Chuyển đổi các mặt thành Uint16Array
  ...
  link.click();
  document.body.removeChild(link); // Xóa link sau khi click
}

console.log(STLExporter);
sixTool.addEventListener("click", () => {
  const object = new MarchingCubesss(volumeData, 64, 64, 64);
  const data = object.run(Number(isovalue.value));
  const dinh = [];
  data.forEach((item) => dinh.push(...item));
  exportSTLFromVerticesAndFaces(dinh, data);
});
